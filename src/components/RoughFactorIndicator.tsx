  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-baseline justify-between">
        <span className="font-medium text-indigo dark:text-white">{label}</span>
        <span className="text-sm text-indigo/60 dark:text-lavender-400/60" aria-hidden="true">
          {zoneLabel}
        </span>
      </div>
      <div
        ...
        {ZONE_LABELS.map((_, i) => (
          <div
            ...
            className={`h-3 flex-1 rounded-full ${
              i === zoneIndex ? FACTOR_COLOR_CLASS[factor] : "bg-lavender-400 dark:bg-white/10"
            }`}
          />
        ))}
      </div>
